...
import { type CreateAgentRequestBody, createAgentRequestBodySchema } from "./schema";
...
export async function POST(request: NextRequest) {
  let requestBody: CreateAgentRequestBody;

  try {
    const json = await request.json();
    requestBody = createAgentRequestBodySchema.parse(json);
  } catch (error) {
...
Make sure every feature has a clear purpose and adds value to the agent. Think about the complete user workflow from start to finish.`,
          experimental_telemetry: {
            ...
            functionId: "create-agent",
          },
          onFinish: async ({ usage }) => {
            console.log('✅ Agent creation completed', usage);
            dataStream.write({
              type: "data-usage",
              data: usage,
            });
    ...
    console.error("Unhandled error in create agent API:", error, { vercelId });
